...
const HATS = [
    ...
    { id: 'crown', emoji: '👑', label: 'Crown', cost: 5 },
    { id: 'tophat', emoji: '🎩', label: 'Top Hat', cost: 10 },
    { id: 'cap', emoji: '🧢', label: 'Cap', cost: 8 },
    { id: 'wizard', emoji: '🪄', label: 'Wizard', cost: 15 },
    { id: 'pirate', emoji: '🏴‍☠️', label: 'Pirate', cost: 20 },
];
...
export function enter(container) {
    ...
    container.innerHTML = `
        ...
        <div style="flex:1;overflow-y:auto;padding:16px;">
            ...
            <div class="card" style="margin-bottom:16px;">
                <h3 style="margin-bottom:12px;">Choose Your Face</h3>
                <div id="face-grid" style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;"></div>
            </div>

            <!-- Hat Selection -->
            <div class="card" style="margin-bottom:16px;">
                <h3 style="margin-bottom:12px;">Choose Your Hat</h3>
    ...
    FACES.forEach(face => {
        const owned = state.totalStars >= face.cost || state.character.face === face.id;
        ...
        if (owned) {
            btn.addEventListener('click', () => {
                ...
                sound.tap();
                enter(container); // Re-render
            });
        }
        if (!owned) {
            btn.title = `${face.cost} stars needed`;
        }
    ...
    HATS.forEach(hat => {
        const owned = state.totalStars >= hat.cost || state.character.hat === hat.id;
        ...
        `;
        btn.innerHTML = `<span>${hat.emoji || '—'}</span><span style="font-size:0.65rem;">${hat.label}${!owned ? '<br>⭐' + hat.cost : ''}</span>`;
        if (owned) {
            btn.addEventListener('click', () => {
